...
import { ResponsiveContainer, LineChart, Line, CartesianGrid, XAxis, YAxis, Tooltip, Label, Legend } from 'recharts';
      ...
      <ChartWrapper disable={data.length === 0}>
        <ResponsiveContainer height={350}>
          <LineChart data={data} margin={{top: 50, left: 30, bottom: 50, right: 50}}>
            {data && data[0] && 
              Object.keys(data[0]).filter(key => key !== 'year').map((key, i) => {
                return (
                  <Line 
                    key={key} 
                    type="monotone" 
